...
if (!['demo', 'test'].includes(config.environment) || config.emailMode !== 'local-outbox') throw new Error('This tool is local-only');
...
try {
  ...
  for (const mail of messages) console.log(`${mail.createdAt.toISOString()} | ${mail.purpose}\n${mail.url}\n`);
