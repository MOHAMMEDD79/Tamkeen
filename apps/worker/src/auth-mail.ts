...
// Links older than this are not worth sending: verification and reset tokens will have expired.
...
const TEMPLATES: Record<string, { subject: string; intro: string; action: string }> = {
  verify: { subject: 'تأكيد بريدك الإلكتروني في تمكين', intro: 'أنشأت حسابًا في تمكين. أكّد بريدك الإلكتروني لتتمكن من تسجيل الدخول.', action: 'تأكيد البريد' },
  reset: { subject: 'إعادة تعيين كلمة المرور في تمكين', intro: 'طلبت إعادة تعيين كلمة المرور. إن لم تطلب ذلك فتجاهل هذه الرسالة.', action: 'تعيين كلمة مرور جديدة' },
...
export function buildAuthMail(row: { recipient: string; purpose: string; url: string }): AuthMailMessage {
...
export async function processAuthMailBatch(db: DatabaseClient, send: AuthMailSender | undefined, now = new Date(), limit = 25) {
  ...
  await db.localAuthMail.updateMany({ where: { state: { in: ['pending', 'failed'] }, createdAt: { lt: new Date(now.getTime() - MAX_AGE_MS) } }, data: { state: 'expired' } });
  let processed = 0;
  while (processed < limit) {
    ...
    try {
      ...
      await db.localAuthMail.update({ where: { id: row.id }, data: { state: 'delivered', deliveredAt: new Date(), lastErrorCode: '' } });
    } catch (error) {
      const attempts = row.attempts + 1;
      await db.localAuthMail.update({ where: { id: row.id }, data: { state: attempts >= 5 ? 'dead_letter' : 'failed', lastErrorCode: error instanceof Error ? error.name.slice(0, 80) : 'delivery_error', nextAttemptAt: new Date(now.getTime() + backoffMs(attempts)) } });
    }
    processed += 1;
  }
  return processed;
