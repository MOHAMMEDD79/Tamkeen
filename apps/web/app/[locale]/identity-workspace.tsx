...
export function IdentityWorkspace({ route, locale }: { route: string; locale: Locale }) {
  ...
  if (route === '/login' || route === '/register') {
    ...
    content = <><h1>التحقق بخطوتين</h1><p>أدخل رمز تطبيق المصادقة لإكمال تسجيل الدخول.</p><form onSubmit={form(async data => { await api('/auth/two-factor/verify-totp', 'POST', { code: data.get('code'), trustDevice: data.get('trust') === 'on' }); window.location.assign(L(returnTo)); })}><Field label="رمز المصادقة" name="code" /><label className="check"><input type="checkbox" name="trust" />تذكّر هذا الجهاز ولا تطلب الرمز عند الدخول منه</label><button type="submit" disabled={busy}>تحقق وتابع</button></form><details><summary>استخدام رمز استرداد</summary><form onSubmit={form(async data => { await api('/auth/two-factor/verify-backup-code', 'POST', { code: data.get('code'), disableSession: false, trustDevice: false }); window.location.assign(L(returnTo)); })}><Field label="رمز الاسترداد" name="code" /><button type="submit" disabled={busy}>استخدام الرمز</button></form></details></>;
    ...
    const staff: Array<{ href: string; title: string; body: string; icon: LucideIcon; show: boolean }> = [
      ...
      { href: '/admin/organizations', title: 'الجهات', body: 'تعديل الجهات وشعاراتها واعتمادها وإظهارها أو إيقافها', icon: Building2, show: me.platformRoles.includes('PlatformAdmin') },
      { href: '/admin/users', title: 'المستخدمون', body: 'كل الحسابات: البيانات والأدوار والإيقاف', icon: UserRound, show: me.platformRoles.includes('PlatformAdmin') },
      { href: '/admin/site/settings', title: 'بيانات التواصل', body: 'البريد والهاتف والعنوان وروابط الشبكات الاجتماعية', icon: Contact, show: me.platformRoles.includes('PlatformAdmin') },
      { href: '/admin/messages', title: 'رسائل التواصل', body: 'ما يرسله الزوار من صفحة تواصل معنا', icon: Inbox, show: me.platformRoles.includes('PlatformAdmin') },
