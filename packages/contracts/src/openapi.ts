...
const schemas = {
  ...
  City: {
    type: 'object',
    properties: { id: { type: 'string', format: 'uuid' }, country: { type: 'string', minLength: 2, maxLength: 2 }, nameAr: { type: 'string' }, nameEn: { type: 'string' }, latitude: { type: 'number', description: 'City centre.' }, longitude: { type: 'number', description: 'City centre.' } }
  },
  CursorPage: {
    type: 'object',
  ...
  AdminOrganization: {
    type: 'object',
    properties: {
      id: { type: 'string', format: 'uuid' }, slug: { type: 'string' }, displayName: { type: 'string' }, legalName: { type: 'string' },
      type: { type: 'string', enum: ['NGO', 'Company', 'Startup', 'Foundation', 'Institution'] }, city: { type: 'string' }, country: { type: 'string' },
      publicDescription: { type: 'string' }, sectors: { type: 'array', items: { type: 'string' } },
      websiteUrl: { type: ['string', 'null'] }, contactEmail: { type: ['string', 'null'] }, contactAddress: { type: ['string', 'null'] },
      verification: { type: 'string' }, publiclyListed: { type: 'boolean' }, status: { type: 'string', enum: ['active', 'suspended', 'closed'] },
      version: { type: 'integer' }, logoUrl: { type: ['string', 'null'] }, createdAt: { type: 'string', format: 'date-time' },
      counts: { type: 'object', properties: { members: { type: 'integer' }, projects: { type: 'integer' }, offerings: { type: 'integer' }, programs: { type: 'integer' }, jobs: { type: 'integer' } } }
    }
  },
  AdminUserSummary: {
    type: 'object',
    properties: {
      id: { type: 'string', format: 'uuid' }, email: { type: 'string', format: 'email' }, name: { type: 'string' }, displayName: { type: 'string' }, city: { type: ['string', 'null'] },
      status: { type: 'string', enum: ['active', 'suspended', 'closed'] }, emailVerified: { type: 'boolean' }, twoFactorEnabled: { type: 'boolean' },
      platformRoles: { type: 'array', items: { type: 'string' } }, organizations: { type: 'integer' }, createdAt: { type: 'string', format: 'date-time' }
    }
  },
  ContactMessage: {
    type: 'object',
...
const authPaths = {
  ...
  '/auth/email-otp/send-verification-otp': { post: operation({ id: 'authResendVerification', summary: 'Queue a fresh 6-digit code for email verification or password reset. The reply never reveals whether the address exists.', tag: 'auth', permission: 'public', public: true, body: { type: 'object', required: ['email', 'type'], properties: { email: { type: 'string', format: 'email' }, type: { type: 'string', enum: ['email-verification', 'forget-password'] } } }, success: { status: 200, description: 'Generic acknowledgement.' }, errors: { 429: errorResponse('Rate limited; retry after the advertised delay.') } }) },
  '/auth/email-otp/verify-email': { post: operation({ id: 'authVerifyEmail', summary: 'Verify the address with the emailed code. Codes expire after 10 minutes, are single-use, and lock after 5 wrong tries.', tag: 'auth', permission: 'token owner', public: true, body: { type: 'object', required: ['email', 'otp'], properties: { email: { type: 'string', format: 'email' }, otp: { type: 'string', pattern: '^[0-9]{6}$' } } }, success: { status: 200, description: 'Address verified; sign in next.' }, errors: { 403: errorResponse('Too many wrong codes; request a new one.'), 429: errorResponse('Rate limited; retry after the advertised delay.') } }) },
  '/auth/email-otp/request-password-reset': { post: operation({ id: 'authRequestPasswordReset', summary: 'Request a password reset code. Rate limited, with a reply that does not disclose the account.', tag: 'auth', permission: 'public', public: true, body: { type: 'object', required: ['email'], properties: { email: { type: 'string', format: 'email' } } }, success: { status: 200, description: 'Generic acknowledgement.' }, errors: { 429: errorResponse('Rate limited; retry after the advertised delay.') } }) },
  '/auth/email-otp/reset-password': { post: operation({ id: 'authResetPassword', summary: 'Set a new password with the emailed code and revoke existing sessions.', tag: 'auth', permission: 'token owner', public: true, body: { type: 'object', required: ['email', 'otp', 'password'], properties: { email: { type: 'string', format: 'email' }, otp: { type: 'string', pattern: '^[0-9]{6}$' }, password: { type: 'string', minLength: 12 } } }, success: { status: 200, description: 'Password replaced and sessions revoked.' }, errors: { 403: errorResponse('Too many wrong codes; request a new one.') } }) },
...
const paths = {
  ...
  '/admin/organizations': { get: operation({ id: 'listAdminOrganizations', summary: 'Every organisation, listed ones first (300 at most), with its public details, approval, listing, status, logo and counts. Optional q searches the names and slug.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', query: [{ name: 'q', description: 'Search text.' }], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'array', items: ref('AdminOrganization') }) } }) },
  '/admin/organizations/{id}': { patch: operation({ id: 'updateAdminOrganization', summary: 'Edit an organisation public details, approve it or withdraw approval, show or hide it in the public directory, or suspend it. A stale version returns 409.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Organisation identifier.')], body: { type: 'object', required: ['version'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, displayName: { type: 'string', minLength: 2, maxLength: 140 }, publicDescription: { type: 'string', maxLength: 1200 }, sectors: { type: 'array', maxItems: 8, items: { type: 'string', maxLength: 40 } }, type: { type: 'string', enum: ['NGO', 'Company', 'Startup', 'Foundation', 'Institution'] }, city: { type: 'string', maxLength: 100 }, country: { type: 'string', pattern: '^[A-Z]{2}$' }, websiteUrl: { type: ['string', 'null'] }, contactEmail: { type: ['string', 'null'] }, contactAddress: { type: ['string', 'null'] }, publiclyListed: { type: 'boolean' }, status: { type: 'string', enum: ['active', 'suspended'] }, approved: { type: 'boolean' } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope(ref('AdminOrganization')) } }) },
  '/admin/organizations/{id}/logo': { put: operation({ id: 'uploadAdminOrganizationLogo', summary: 'Replace an organisation logo with the raw PNG or JPEG body (max 2 MB, 4096x4096). The same quarantine and content checks as an organisation upload apply. Demo/test storage only.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Organisation identifier.')], headers: [{ name: 'content-length', description: 'Exact byte size.' }, { name: 'content-type', description: 'image/png or image/jpeg.' }], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope(ref('AdminOrganization')) } }) },
  '/admin/users': { get: operation({ id: 'listAdminUsers', summary: 'Accounts, newest first (300 at most), with status, verification, 2FA, platform roles and organisation count. Optional q searches email and names; optional status filters.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', query: [{ name: 'q', description: 'Search text.' }, { name: 'status', description: 'active, suspended or closed.' }], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'array', items: ref('AdminUserSummary') }) } }) },
  '/admin/users/{id}': { get: operation({ id: 'getAdminUser', summary: 'One account: profile, platform roles, organisation memberships and the last sign-in time.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'User identifier.')], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object' }) } }) },
  '/admin/users/{id}/status': { post: operation({ id: 'setAdminUserStatus', summary: 'Suspend or reactivate an account. A suspended account cannot sign in and its sessions stop at once. An admin cannot suspend themselves (403) or the last active platform admin (409).', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'User identifier.')], body: { type: 'object', required: ['status'], additionalProperties: false, properties: { status: { type: 'string', enum: ['active', 'suspended'] } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { id: { type: 'string', format: 'uuid' }, status: { type: 'string' } } }) } }) },
  '/listing-covers': { get: operation({ id: 'getListingCovers', summary: 'Admin-set cover photos of offerings, programmes and jobs, each keyed by the listing slug. A listing absent here uses its default topic photo.', tag: 'site', permission: 'public', public: true, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { offering: { type: 'object', additionalProperties: { type: 'string' } }, program: { type: 'object', additionalProperties: { type: 'string' } }, job: { type: 'object', additionalProperties: { type: 'string' } } } }) } }) },
  '/admin/listings': { get: operation({ id: 'listAdminListings', summary: 'Every project, offering, programme and job (300 of each at most, newest first) with its state, organisation and cover photo or null.', tag: 'site', permission: 'PlatformAdmin grant with MFA', success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { projects: { type: 'array', items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, slug: { type: 'string' }, title: { type: 'string' }, state: { type: 'string' }, organization: { type: 'object', properties: { displayName: { type: 'string' }, status: { type: 'string' } } }, coverUrl: { type: ['string', 'null'] } } } }, offerings: { type: 'array', items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, slug: { type: 'string' }, title: { type: 'string' }, state: { type: 'string' }, organization: { type: 'object', properties: { displayName: { type: 'string' }, status: { type: 'string' } } }, coverUrl: { type: ['string', 'null'] } } } }, programs: { type: 'array', items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, slug: { type: 'string' }, title: { type: 'string' }, state: { type: 'string' }, organization: { type: 'object', properties: { displayName: { type: 'string' }, status: { type: 'string' } } }, coverUrl: { type: ['string', 'null'] } } } }, jobs: { type: 'array', items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, slug: { type: 'string' }, title: { type: 'string' }, state: { type: 'string' }, organization: { type: 'object', properties: { displayName: { type: 'string' }, status: { type: 'string' } } }, coverUrl: { type: ['string', 'null'] } } } } } }) } }) },
  '/admin/listings/{kind}/{id}': { patch: operation({ id: 'updateAdminListing', summary: 'Edit any listing public text, or hide, remove or restore it. visibility changes only what the public sees, never the workflow state; removed listings keep their contributions, applications and history. A stale version returns 409; a field the kind does not have returns 422.', tag: 'site', permission: 'PlatformAdmin grant with MFA', params: [tokenParam('kind', 'project, offering, program or job.'), uuidParam('id', 'Listing identifier.')], body: { type: 'object', required: ['version'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, title: { type: 'string', minLength: 3, maxLength: 200 }, summary: { type: 'string', maxLength: 2000 }, story: { type: 'string', maxLength: 20000 }, responsibilities: { type: 'string', maxLength: 4000 }, requirements: { type: 'string', maxLength: 4000 }, visibility: { type: 'string', enum: ['visible', 'hidden', 'removed'] } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { kind: { type: 'string' }, id: { type: 'string', format: 'uuid' }, visibility: { type: 'string' } } }) } }) },
  '/admin/listings/{kind}/{id}/cover': {
    put: operation({ id: 'setListingCover', summary: 'Set the cover photo of an offering, programme or job from an uploaded image key. Projects use /admin/projects/{id}/cover.', tag: 'site', permission: 'PlatformAdmin grant with MFA', params: [tokenParam('kind', 'offering, program or job.'), uuidParam('id', 'Listing identifier.')], body: { type: 'object', required: ['imageKey'], additionalProperties: false, properties: { imageKey: { type: 'string', pattern: '^site/[0-9a-f-]{36}[.]bin$' } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { kind: { type: 'string' }, subjectId: { type: 'string', format: 'uuid' }, coverUrl: { type: 'string' } } }) } }),
    delete: operation({ id: 'removeListingCover', summary: 'Remove a listing cover photo; the listing goes back to its default topic photo. 404 when it had none.', tag: 'site', permission: 'PlatformAdmin grant with MFA', params: [tokenParam('kind', 'offering, program or job.'), uuidParam('id', 'Listing identifier.')], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { kind: { type: 'string' }, subjectId: { type: 'string', format: 'uuid' }, coverUrl: { type: 'null' } } }) } })
  },
  '/admin/projects': { get: operation({ id: 'listAdminProjects', summary: 'Every project in all three tracks (500 at most, newest first) with its state, visibility, organisation, city, goal, raised amount and counts. Optional type and q.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', query: [{ name: 'type', description: 'charity, venture or enablement.' }, { name: 'q', description: 'Search text.' }], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'array', items: { type: 'object' } }) } }) },
  '/admin/projects/{id}': {
    get: operation({ id: 'getAdminProject', summary: 'One project with everything the admin can change.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }),
    patch: operation({ id: 'updateAdminProject', summary: 'Edit a project text, track, city, location precision, state, state reason or public visibility. The track cannot change once money is attached (409); a stale version returns 409.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], body: { type: 'object', required: ['version'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, title: { type: 'string', maxLength: 140 }, summary: { type: 'string', maxLength: 300 }, story: { type: 'string', maxLength: 20000 }, type: { type: 'string', enum: ['charity', 'venture', 'enablement'] }, cityId: { type: 'string', format: 'uuid' }, publicLocationPrecision: { type: 'string', enum: ['exact', 'approximate', 'city'] }, latitude: { type: ['number', 'null'], minimum: -90, maximum: 90 }, longitude: { type: ['number', 'null'], minimum: -180, maximum: 180 }, state: { type: 'string', enum: ['draft', 'published', 'paused', 'funding_closed', 'executing', 'impact_review', 'completed', 'archived', 'cancelled'] }, stateReason: { type: 'string', maxLength: 1000 }, visibility: { type: 'string', enum: ['visible', 'hidden', 'removed'] } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }),
    delete: operation({ id: 'deleteAdminProject', summary: 'Delete a project that never touched money, with its drafts, reviews, budget, stages, updates and reports. A project with contributions, payouts, pools, agreements or programmes returns 409: remove it from the site instead.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { id: { type: 'string', format: 'uuid' }, deleted: { type: 'boolean' } } }) } })
  },
  '/admin/projects/{id}/campaign': { put: operation({ id: 'saveAdminProjectCampaign', summary: 'Set a project funding goal and end date (and, before publication, its currency and policy). The policy and currency cannot change after publication (409).', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], body: { type: 'object', required: ['version', 'goalMinor', 'endsAt'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, goalMinor: { type: 'string', pattern: '^[1-9][0-9]{0,15}$' }, currency: { type: 'string', pattern: '^[A-Z]{3}$' }, policy: { type: 'string', enum: ['flexible', 'all_or_nothing'] }, endsAt: { type: 'string', format: 'date-time' } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }) },
  '/admin/projects/{id}/budget': { put: operation({ id: 'saveAdminProjectBudget', summary: 'Replace a project budget lines. The previous budget is kept as a revision; a published project needs a reason of 10+ characters.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], body: { type: 'object', required: ['version', 'reason', 'lines'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, reason: { type: 'string', maxLength: 1000 }, lines: { type: 'array', minItems: 1, maxItems: 60, items: { type: 'object', required: ['label', 'amountMinor'], additionalProperties: false, properties: { label: { type: 'string', maxLength: 140 }, amountMinor: { type: 'string', pattern: '^[1-9][0-9]{0,15}$' } } } } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }) },
  '/admin/projects/{id}/milestones': { put: operation({ id: 'saveAdminProjectMilestones', summary: 'Replace a project stages; weights must total 100. Refused (409) once any stage carries evidence or a payout.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.')], body: { type: 'object', required: ['version', 'milestones'], additionalProperties: false, properties: { version: { type: 'integer', minimum: 1 }, milestones: { type: 'array', minItems: 1, maxItems: 40, items: { type: 'object', required: ['title', 'budgetMinor', 'weight'], additionalProperties: false, properties: { title: { type: 'string', maxLength: 140 }, budgetMinor: { type: 'string', pattern: '^[1-9][0-9]{0,15}$' }, weight: { type: 'integer', minimum: 1, maximum: 100 } } } } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }) },
  '/admin/projects/{id}/updates/{updateId}': {
    patch: operation({ id: 'editAdminProjectUpdate', summary: 'Correct a published project update.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.'), uuidParam('updateId', 'Update identifier.')], body: { type: 'object', required: ['title', 'body'], additionalProperties: false, properties: { title: { type: 'string', maxLength: 140 }, body: { type: 'string', maxLength: 20000 } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } }),
    delete: operation({ id: 'deleteAdminProjectUpdate', summary: 'Delete a published project update.', tag: 'admin', permission: 'PlatformAdmin grant with MFA', params: [uuidParam('id', 'Project identifier.'), uuidParam('updateId', 'Update identifier.')], success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', description: 'The project with its campaign, budget lines, milestones, updates, raised amount, money ties and whether it can be deleted.' }) } })
  },
  '/orgs/{id}/projects/{projectId}/location': { put: operation({ id: 'updateProjectLocation', summary: 'Move a project pin on the map, in any state. Needs project.update in the organisation. City precision stores no point; approximate and exact need latitude and longitude. A stale version returns 409.', tag: 'projects', permission: 'project.update', params: [uuidParam('id', 'Organisation identifier.'), uuidParam('projectId', 'Project identifier.')], body: { type: 'object', required: ['publicLocationPrecision', 'version'], additionalProperties: false, properties: { publicLocationPrecision: { type: 'string', enum: ['exact', 'approximate', 'city'] }, latitude: { type: ['number', 'null'] }, longitude: { type: ['number', 'null'] }, version: { type: 'integer', minimum: 1 } } }, success: { status: 200, description: ENVELOPE_DESCRIPTION, schema: envelope({ type: 'object', properties: { version: { type: 'integer' }, publicLocationPrecision: { type: 'string' }, latitude: { type: ['number', 'null'] }, longitude: { type: ['number', 'null'] } } }) } }) },
